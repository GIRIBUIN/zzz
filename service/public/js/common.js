...
function feedbackUrl(user = requireCurrentUser()) {
...
async function renderDeviceStatus(user) {
  ...
  try {
    renderDeviceInfo(await fetchMyDevice(user));
  } catch (error) {
    statusEl.textContent = "상태 확인 실패";
    badgeEl.textContent = "Error";
    badgeEl.className = "health-badge disconnected";
...
function renderAuthPanel() {
  ...
  document.getElementById("deviceRegisterBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      setAuthNotice(result.action === "created" ? "기기를 등록했습니다." : "이미 등록된 기기입니다.");
    } catch (error) {
      setAuthNotice(`기기 등록 실패: ${error.message}`, "error");
      await renderDeviceStatus(user);
    } finally {
      registerBtn.disabled = false;
    }
  });
